 ...
 * outputs through the real pipeline), and the API/worker run WITHOUT a fal key, so no paid call is possible.
...
const serverEnv = {
  ...
  PORT: String(apiPort), CLIENT_ORIGIN: baseURL, FAL_KEY: '',
};
...
export default defineConfig({
  ...
  webServer: [
    {
      command: `${tsx} server/src/decomposition/e2eSeed.ts && npx concurrently -k -n api,worker "${tsx} server/src/index.ts" "${tsx} server/src/decomposition/workerEntry.ts"`,
