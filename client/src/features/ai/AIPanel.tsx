...
function GeneratePanel() {
  ...
  return <div className="ai-panel">
    <div className="ai-panel-scroll">
      ...
      <details className="event-fields" open><summary>Exact event wording <span>Editable text</span></summary><p className="inspector-hint">These optional words become editable text and stay exact when adapting. They are not sent to the image model.</p>
        {(Object.keys(content) as (keyof typeof content)[]).map((role) => <label key={role}><span className="control-label">{role[0].toUpperCase() + role.slice(1)}</span>{role === 'date'
          ...
          : <textarea aria-label={`Event ${role}`} value={content[role]} maxLength={TEXT_LIMITS.maxCharacters} disabled={busy} onChange={(event) => setContent({ ...content, [role]: event.target.value })} rows={role === 'venue' ? 2 : 1} />}</label>)}
      </details>
      <div ref={feedback}>
    ...
    <div className="ai-panel-actions">
      {ai.preview ? <><button className="button primary-button" onClick={apply}>Use this design<ArrowUpRight size={14} /></button><div className="ai-secondary-actions"><button className="button" disabled={configured !== true} onClick={() => void generate()}>Regenerate</button><button className="button" onClick={() => void discard()}>Discard</button></div></>
