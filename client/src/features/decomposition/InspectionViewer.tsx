...
export function InspectionViewer({ job }: { job: DecompositionJobSummary }) {
  ...
  const groups = ['01-original', '02-analysis', '03-qwen', '04-sam2', '05-refined'];
  const labels = ['Original', 'Analysis', 'Qwen proposals', 'SAM2 candidates', 'Refined masks and alpha'];
  const latest = [...new Map(job.artifacts.map((artifact) => [artifact.relativePath, artifact])).values()];
  return <section aria-label="Phase 1 to 5 inspection">
    ...
    {groups.map((group, i) => <details key={group} open={i === Math.min(job.phase - 1, 4)}><summary>{labels[i]}</summary>
