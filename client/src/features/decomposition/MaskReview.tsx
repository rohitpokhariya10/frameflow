...
export function MaskReview({ job, candidates, sourceId, width, height, onSubmit, busy }: {
  ...
  return <section aria-label="Mask review">
    ...
    <div className="decomp-row"><label>Object <select value={active} onChange={(e) => setActive(e.target.value)}>{candidates.map((c) => <option key={c.id} value={c.id}>{c.source === 'synthesized' ? 'SYNTHESIZED' : c.source === 'sam3' ? 'REFINED' : 'RAW'}: {c.label} — {c.id} ({((c.statistics?.areaFraction ?? 0) * 100).toFixed(1)}% of image)</option>)}</select></label>
    ...
    {candidate && <><p><strong>{candidate.id}</strong> · {job.artifacts.find(a => a.artifactId === candidate.overlayArtifactId)?.relativePath || 'Saved candidate'}<br />{candidate.source === 'synthesized' ? `Synthesized from ${candidate.sourceCandidateIds?.join(', ')}; Qwen ${candidate.proposalId}. Grouping requires review.` : `Raw source support. Qwen matches: ${candidate.proposalMatches?.map(match => match.proposalId).join(', ') || 'none reliable'}.`}<br />Cyan is selected ownership; the remaining source is context only. Renaming does not expand a mask.</p><a href={artifactUrl(candidate.maskArtifactId)} target="_blank" rel="noreferrer">Inspect actual black/white mask</a></>}
    <p>Positive = should belong, including missing regions. Negative = should not belong. Refine with guidance runs one targeted attempt and returns a mask for inspection.</p>
