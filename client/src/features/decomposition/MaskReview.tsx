...
export function MaskReview({ job, candidates, sourceId, width, height, onSubmit, busy }: {
  ...
  const [objects, setObjects] = useState<NonNullable<DecompositionReview['objects']>>(() => {
    ...
    // Strokes from an already-processed submission are baked into the current revision; re-sending them would re-apply
    // old edits to a new mask. Only an unsent draft of this exact revision keeps its strokes. Points remain as guidance.
    const processed = saved === job.reviewSubmission;
    return candidates.map(c => { const restored = saved?.objects?.find(object => object.id === c.id && (object.candidateId ?? object.id) === c.id); return { id: c.id, candidateId: c.id, label: c.label, selected: false, points: [], strokes: [], completeHidden: false, ...restored, ...(processed ? { strokes: [] } : {}) }; });
  });
  useEffect(() => {
  ...
  return <section aria-label="Mask review">
    ...
    {candidate?.target && <div aria-label="Ownership status"><p>Target: {selected?.label} · <strong>Status: {candidate.qualityTier ?? (candidate.qualityStatus === 'needs-correction' ? 'FAIL' : 'REVIEW')}</strong> · {candidate.qualityTier === 'FAIL' || candidate.qualityStatus === 'needs-correction' ? 'Needs correction before edge refinement' : candidate.qualityTier === 'PASS' ? 'Checks passed; confirm ownership' : 'Needs visual confirmation'}</p>
      {candidate.qualityChecks?.length ? <ul>{candidate.qualityChecks.map(check => <li key={check.code}>{check.tier === 'FAIL' ? 'Must fix: ' : 'Check: '}{check.message}</li>)}</ul> : null}</div>}
    {advanced && candidate && <><p>Provider: SAM 3.1 · Revision {candidate.revisionId || job.revision}{candidate.warnings.length ? ` · ${candidate.warnings.join(', ')}` : ''}</p><p><strong>{candidate.id}</strong> · {job.artifacts.find(a => a.artifactId === candidate.overlayArtifactId)?.relativePath || 'Saved candidate'}<br />{candidate.source === 'sam3' ? 'Source-image semantic ownership; generated RGB is not used.' : candidate.source === 'synthesized' ? `Synthesized from ${candidate.sourceCandidateIds?.join(', ')}; Qwen ${candidate.proposalId}. Grouping requires review.` : `Raw source support. Qwen matches: ${candidate.proposalMatches?.map(match => match.proposalId).join(', ') || 'none reliable'}.`}<br />Cyan is selected ownership; the remaining source is context only. Renaming does not expand a mask.</p><a href={artifactUrl(candidate.maskArtifactId)} target="_blank" rel="noreferrer">Inspect actual black/white mask</a></>}
    ...
    <p>Positive = should belong, including missing regions. Negative = should not belong. Refine with AI runs one targeted attempt and returns a new mask for inspection; the current mask is only a hint, so re-apply brush edits afterwards if needed.</p>
