...
function VariantCard({ variant, label }: { variant: DesignVariant; label: string }) {
  ...
  return <section className="variant-card" aria-label={`${label} version`}>
    <div className="variant-card-heading"><div><strong>{label}</strong><small>{label === 'Source' ? 'Source kept' : 'New composition'}</small></div><span>{formatLabel(variant.canvas)}<br />{variant.canvas.width} × {variant.canvas.height}</span></div>
    <div className="variant-card-body" ref={holder}><div data-testid={`${label.toLowerCase()}-frame`} className={variant.canvas.transparent ? 'is-transparent-canvas' : undefined} style={{ width: variant.canvas.width * zoom, height: variant.canvas.height * zoom }}>
