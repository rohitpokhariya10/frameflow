...
export const initialHistory = (document: ProjectDocument): HistoryState => ({ document, past: [], future: [], group: null });
...
export function historyReducer(state: HistoryState = initialHistory(editorSlice.getInitialState().document), action: UnknownAction): HistoryState {
  ...
  if (undo.match(action)) {
    ...
    return document ? { document, past: state.past.slice(0, -1), future: [state.document, ...state.future].slice(0, HISTORY_LIMIT), group: null } : state;
  }
  if (redo.match(action)) {
    const document = state.future[0];
    return document ? { document, past: [...state.past, state.document].slice(-HISTORY_LIMIT), future: state.future.slice(1), group: null } : state;
  }
  ...
  return { document, past: coalesce ? state.past : [...state.past, state.document].slice(-HISTORY_LIMIT), future: [], group };
}
