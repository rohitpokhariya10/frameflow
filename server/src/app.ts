...
import { validGenerateRequest, type ImageProvider } from '@frameflow/shared';
import { geminiProvider } from './providers/geminiProvider.js';
import { cloudflareImageProvider, CLOUDFLARE_IMAGE_MODEL } from './providers/cloudflareImageProvider.js';
...
export function createApp(config: ServerConfig, provider?: GenerateImage, log: (event: object) => void = console.info) {
  ...
  app.post('/api/ai/generate', limiter, async (req, res, next) => {
    ...
    try {
      ...
      const result = await generateArtwork(req.body, requestId, config.model, config.timeoutMs, generate, controller.signal, config.provider);
